...
inherit autotools toolchain-funcs cmake-multilib
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${P}-install-cmake-exports.patch
...
#	esac
	local mycmakeargs=(
		-DINSTALL_BIN_DIR:PATH=${EPREFIX%/}/usr/bin
		-DINSTALL_INC_DIR:PATH=${EPREFIX%/}/usr/include
		-DINSTALL_LIB_DIR:PATH=${EPREFIX%/}/usr/$(get_libdir)
		-DINSTALL_MAN_DIR:PATH=${EPREFIX%/}/usr/share/man
		-DINSTALL_PKGCONFIG_DIR:PATH=${EPREFIX%/}/usr/$(get_libdir)/pkgconfig
	)
	...
	if use minizip ; then
		...
		LDFLAGS="${LDFLAGS} -L../.." CPPFLAGS="${CPPFLAGS} -I../.." econf $(use_enable static-libs static)
	fi
